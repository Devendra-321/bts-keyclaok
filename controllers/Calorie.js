...
module.exports.createCalorie = function createCalorie(req, res, next) {
  ...
  calorieService.createCalorie(req, res, next);
};
...
/**
 * Creates bulk calorie
 *
 * @param {ClientRequest} req - The http request object
 * @param {IncomingMessage} res - The http response object
 * @param {function} next - The callback used to pass control to the next action/middleware
 */
 module.exports.bulkCreateCalorie = function bulkCreateCalorie(req, res, next) {
  let calorieService = new CalorieService(req.Logger);
  calorieService.bulkCreateCalorie(req, res, next);
};